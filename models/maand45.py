...
class Maand45(Base):
    ...
    dropjump_rsi_l = Column(DECIMAL(3,1), nullable=True)
    dropjump_rsi_r = Column(DECIMAL(3,1), nullable=True)
    ...
    # === Revalidatie-info ===
    lopen_opstartdatum = Column(Date, nullable=True)
    aantal_sessies = Column(Integer, nullable=True)

    # === Relatie ===
    blessure = relationship("Blessure", back_populates="maand45", passive_deletes=True)
